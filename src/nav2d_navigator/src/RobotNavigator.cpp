...
{	
	...
	ROS_INFO("Inside Robot Navigator");
	...
	navigatorNode.param("exploration_strategy", mExplorationStrategy, std::string("RankBasedExtention"));
...
{	
	...
	if(mCellInflationRadius == 0)
	{
		...
		ROS_INFO("Setting the clock %ld", time_in_seconds);
...
{	
	ROS_DEBUG("Inside Create Plan method");
...
{	
	...
	while(true)
	{
		...
		if(!setCurrentPosition())
		{
			...
			stop();
			time_in_seconds = time (NULL);
			ROS_INFO("Time now: %ld", time_in_seconds);
			return;
		}
		...
		if(reCheck || nearGoal)
		{
			...
			if(preparePlan())
			{
				...
				//ROS_INFO("Value of result %d", result);
				switch(result)
				{
